...
function countTabsBeforeSubstring(
	str: string,
	sub: string,
	tabChar: string,
): number {
...
/**
 * Resolves an import path based on the file path, import path, and configuration.
 * @param filePath The path of the file doing the import.
...
export function resolveImportPath(
	...
	if (!config.path) return resolvePath(`${filePath}/${importPath}`);
	...
	if (useWildcard) {
		...
		if (!pathAlias) {
			log.error(
				`Path \x1b[33m${importPath}\x1b[0m not found in path aliases. Aborting...`,
			);
			return "";
		}
		...
		if (pathAlias.startsWith("~")) {
			return resolvePath(
				`${config.source}/${pathAlias.replace("~/", "").replace("~", "")}${dynPath}`,
			);
		}
		...
		if (pathAlias.startsWith("~")) {
			return resolvePath(
				`${config.source}/${pathAlias.replace("~/", "").replace("~", "")}`,
			);
		}
...
export async function getModuleUsage(
	module: ScaffModuleStore,
	fileGroup: ScaffFileGroup,
	file: ScaffFile,
	config: ScaffConfig,
): Promise<ScaffModuleUsage[] | null> {
	const matches = [...file.content.matchAll(modControlRegex)]
		.filter((match) => match.groups!.cmd !== "export");

	const limit = 10;
	const results: ScaffModuleUsage[] = [];
	let isInvalid = false;

	// set results for regular imports & includes
	for (let i = 0; i < matches.length; i += limit) {
		...
		const res = await Promise.all(
			batch.map(async (match) => {
				...
				if (!(cmd && mod /*&& src*/ && path)) {
					log.error(
						`Invalid module control statement: \x1b[34m${cmd} ${mod} from ${path}\x1b[0m in \x1b[33m${file.name}\x1b[0m from \x1b[32m${file.path}\x1b[0m. Aborting...`,
					);
					isInvalid = true;
					return null;
				}
				...
				const fromPath = normalizePath(
					resolveImportPath(file.path, path.slice(1, -1), config),
				);
					...
					if (
						...
						for (const f of files) {
							...
							const targetFile = fileGroup.normal.find((fl) => {
								...
								if (!targetFile.endsWith(".gml")) targetFile += ".gml";
								if (!currFile.endsWith(".gml")) currFile += ".gml";
							...
							if (targetFile) filePaths.push(targetFile);
								...
								if (config.onNotFound === "error") {
									...
									log.warn(
										`File \x1b[33m${f.slice(1, -1)}\x1b[0m from \x1b[32m${file.path}\x1b[0m not found. Skipping this file...`,
									);
							}
						}
				...
				if (mod === "*" || (mod.startsWith("{") && mod.endsWith("}"))) {
					...
					Object.entries(module[fromPath]).forEach(([key, value]) => {
						if (mod === "*")
							...
							if (usingAlias) {
								if (!module[fromPath]) module[fromPath] = {};
					...
					if (!module[fromPath][mod]) {
						if (config.onNotFound === "error") {
							log.error(
								`Module \x1b[33m${mod}\x1b[0m from \x1b[32m${fromPath}\x1b[0m not found. Aborting...`,
							);
							isInvalid = true;
						} else
							log.warn(
								`Module \x1b[33m${mod}\x1b[0m from \x1b[32m${fromPath}\x1b[0m not found. Skipping this module...`,
							);
...
export async function implementModules(
	module: ScaffModuleStore,
	fileGroup: ScaffFileGroup,
	file: ScaffFile,
	config: ScaffConfig,
	mods?: ScaffModuleUsage[] | null,
) {
	if (!mods) mods = await getModuleUsage(module, fileGroup, file, config);
	...
	// only implement includes & imports
	for (const mod of mods) {
		if (!mod) return null;

		if (!mod.cmd) return null;

		switch (mod.cmd) {
			case "include":
				let toReplace = "";
				// console.log(`mod.modList: ${JSON.stringify(mod.modList, null, 2)}`);

				if (!mod.files) {
					if (!mod.modList) return null;

					const modEntries = Object.entries(mod.modList);
					const modLen = modEntries.length;
					const modIterator = modEntries.entries();

					// console.log(`modEntries: ${JSON.stringify(modEntries, null, 2)}`);
					...
					for (const fileOrPath of mod.files) {
						if (typeof fileOrPath === "string") {
							...
							toReplace += content + "\n\n";
						} else toReplace += fileOrPath.content + "\n\n";
					}
			...
			case "import":
				...
				for (const match of cmdMatches) {
					...
					if (!contentCmd || !contentMod) {
						log.error(
							`Invalid content module statement: \x1b[34m${contentCmd} ${contentMod}\x1b[0m in \x1b[33m${file.name}\x1b[0m from \x1b[32m${file.path}\x1b[0m. Aborting...`,
						);
						return null;
					}
					...
					if (config.debugLevel >= 1)
						log.debug(
							`Content module statement found: \x1b[34m${contentCmd} ${contentMod}\x1b[0m in \x1b[33m${file.name === "" ? "index" : file.name}\x1b[0m from \x1b[32m${file.path}\x1b[0m.`,
						);
							...
							const tabChar =
								config.tabType === "1t"
									...
									: config.tabType === "2s"
										? "  "
										: "    ";
							const tabCnt = countTabsBeforeSubstring(
								file.content,
								match[0]!,
								tabChar,
							);
							...
							if (tabCnt > 0) {
								...
								file.content = file.content.replace(
									...
									parsedStr
										...
										.map((l, i) =>
											tabLevels[i]
												? insertTabs(tabLevels[i], config.tabType) + l
												: l,
										)
										.join("\n"),
								);
							} else file.content = file.content.replace(match[0]!, parsedStr);
							break;
				...
				for (const match of shortCmdMatches) {
					...
					if (!contentMod) {
						log.error(
							`Invalid content module statement: \x1b[34m@:${contentMod}\x1b[0m in \x1b[33m${file.name}\x1b[0m from \x1b[32m${file.path}\x1b[0m. Aborting...`,
						);
						return null;
					}
					...
					if (config.debugLevel >= 1)
						log.debug(
							`Valueof module statement found: \x1b[34m${contentMod}\x1b[0m in \x1b[33m${file.name}\x1b[0m from \x1b[32m${file.path}\x1b[0m.`,
						);
				...
				for (const match of useMatches) {
					...
					if (!contentMod || !body) {
						log.error(
							`Invalid content module statement: \x1b[34m@use ${contentMod} ${body}\x1b[0m in \x1b[33m${file.name}\x1b[0m from \x1b[32m${file.path}\x1b[0m. Aborting...`,
						);
						return null;
					}
					...
					if (config.debugLevel >= 1)
						log.debug(
							`Use module statement found: \x1b[34m@use ${contentMod} ${body}\x1b[0m in \x1b[33m${file.name}\x1b[0m from \x1b[32m${file.path}\x1b[0m.`,
						);
					...
					const tabLevels = getTabLevels(body, config.tabType).map(
						(l) => l + tabCnt,
					);

					const currMod = module[mod.targetPath!]![
						`@${contentMod}` in module[mod.targetPath!]!
							? `@${contentMod}`
							: contentMod
					]! as ScaffModuleInterface;
					const modMember = Object.entries(currMod.member);
					const tabLevel =
						tabCnt + tabLevels[Math.min(1, tabLevels.length - 1)]!;
					...
					for (const [idx, [mName, mVal]] of modMember.entries()) {
						...
						if (pairIdx > -1) swapAndPop(pairs, pairIdx);
					}

					for (const [idx, pair] of pairs.entries()) {
						if (idx === 0) res = res.slice(0, -1) + ",\n";
